...
<div class="t-card overflow-hidden">
	<div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
		<div>
			<h2 class="text-base font-semibold text-gray-800 mb-1">Script Preview</h2>
			<p class="text-xs text-gray-500">
				{parseResult.metadata.validLines} valid / {parseResult.metadata.totalLines} total · {parseResult
					.metadata.speakers.length} speaker{parseResult.metadata.speakers.length !== 1 ? 's' : ''}
			</p>
		</div>
		<div>
			{#if parseResult.isValid}
				<span class="px-2 py-0.5 border border-green-400 text-green-700 text-xs">valid</span>
			{:else}
				<span class="px-2 py-0.5 border border-red-400 text-red-700 text-xs">invalid</span>
			{/if}
		</div>
	</div>
	...
	<div class="px-4 py-2 border-b border-gray-200 flex flex-wrap gap-1.5">
		{#each parseResult.metadata.speakers as speaker}
			<span class="px-1.5 py-0.5 border border-gray-300 text-xs text-gray-700">
		...
		{#each parseResult.slides as slide}
			<div class="px-4 py-2 hover:bg-gray-50 transition-colors">
				<div class="flex items-center gap-2 mb-0.5">
					<span class="text-xs text-gray-400 w-6">#{slide.lineNumber}</span>
					<span class="text-gray-700 text-xs font-medium">{slide.speaker.name}</span>
	...
	{#if parseResult.errors.length > 0}
		<div class="px-4 py-2 border-t border-gray-200 border-l-2 border-l-red-300 bg-white">
			<p class="text-xs text-red-700 mb-1">
				[error] {parseResult.errors.length} line{parseResult.errors.length !== 1 ? 's' : ''} could not
				be parsed:
			</p>
			<div class="max-h-32 overflow-y-auto space-y-0.5">
				...
				{#if parseResult.errors.length > 5}
					<p class="text-xs text-gray-400">...and {parseResult.errors.length - 5} more</p>
				{/if}
			</div>
		</div>
	{/if}
