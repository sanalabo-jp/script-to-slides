<script lang="ts">
  ...
  function handleAddSpeaker(name: string, role: string) {
  ...
  function handleUpdateMessage(id: string, dialogue: string, visualHint: string) {
    messages = messages.map((m) =>
      m.id === id ? { ...m, dialogue, visualHint, isEditing: false } : m
    );
  }
...
<div class="space-y-3">
  ...
  {:else}
    ...
    <div class="t-card px-4 py-2 border-t-2 border-gray-900">
      <div class="flex items-center gap-2">
        <span class="text-xs font-mono text-gray-400">[locked]</span>
        <span class="text-xs font-mono text-gray-700 font-medium">{frontMatter?.topic}</span>
        {#if frontMatter?.categories && frontMatter.categories.length > 0}
          <span class="text-xs text-gray-300">|</span>
          <span class="text-xs font-mono text-gray-500">{frontMatter.categories.join(', ')}</span>
        {/if}
      </div>
    </div>

    <!-- Chat area -->
    <div class="t-card p-3 min-h-48 max-h-96 overflow-y-auto space-y-2">
      {#if messages.length === 0}
        <p class="text-center text-xs text-gray-400 py-8 font-mono">No messages yet. Start typing below.</p>
      {:else}
    ...
    <ChatInputArea
      {currentSpeakerId}
      {speakers}
      onSpeakerChange={(id) => { currentSpeakerId = id; }}
      onSubmit={handleSubmitMessage}
      onAddSpeaker={handleAddSpeaker}
      onRemoveSpeaker={handleRemoveSpeaker}
      onMetadataClick={() => { showMetadataModal = true; }}
      hasPendingMetadata={hasPendingMeta}
    />

    <!-- Error -->
    {#if errorMsg}
      <p class="text-xs font-mono text-red-600">[error] {errorMsg}</p>
    {/if}
  ...
  {#if showMetadataModal}
    <MetadataModal
      ...
      onCancel={() => { showMetadataModal = false; }}
    />
  {/if}
</div>
