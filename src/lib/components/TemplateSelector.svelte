...
<div class="t-card overflow-hidden">
  <div class="px-4 py-3 border-b border-gray-200">
    <h2 class="text-sm font-mono font-semibold text-gray-800">Select Template</h2>
    <p class="text-xs text-gray-500 mt-0.5">Choose a design template for your presentation</p>
  </div>
  ...
  <div class="p-4 grid grid-cols-3 gap-3">
    {#each templatePresets as template}
      ...
      >
        ...
        <!-- Template info -->
        <p class="font-mono text-xs font-medium text-gray-800">{template.name}</p>
        <p class="text-xs text-gray-500 mt-0.5">{template.description}</p>

        <!-- Selection indicator -->
        {#if isSelected}
          <div class="absolute top-1.5 right-1.5 text-xs font-mono text-gray-900 font-bold">[*]</div>
        {/if}
      </button>
    {/each}
  </div>
